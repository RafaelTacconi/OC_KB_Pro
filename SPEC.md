...
- A64. `parse_xlsx` does not assume physical row 1 is the header: it identifies the real header row, counts only the data rows after it, and renders each sheet with an explicit 1-based row-number column, so the data-row count is visible and checkable in the rendered output.
- A65. When a sheet's header row cannot be determined confidently, the renderer falls back to the current behaviour (first row as header) **and** states the uncertainty in the rendered sheet text, so both the reader and the model can see the header row was not determined confidently.
...
- A77. Every `chunk_id` is regenerated by the pass; a citation stored in existing chat history that no longer resolves is handled by the UI without crashing, in a defined and visible way, and any note referring to a chunk by id (including the ids in `memory.md`) is treated as stale.
...
- **"Confident" must be defined in checkable terms**, not asserted. The spec states the concrete
  property that makes a row the confident header row (a rule expressed over where non-empty cells
  occur), so a reader can tell whether a given sheet was handled confidently.
...
- **If removal would empty a page entirely**, the spec defines the behaviour so that "no text
  discarded" (§22.3) is not violated — the page's text is not discarded; the furniture is retained
  for that page or the page is preserved some other defined way.
...
- **Every `chunk_id` is regenerated.** Consequences the spec must state: any citation stored in
  existing chat history points at chunk ids that no longer exist; and any note that refers to a
  chunk by id — **including the ids recorded in `memory.md`** — becomes stale. **The UI must handle
  a stored citation that no longer resolves** in a defined, non-crashing way, and the spec states
  exactly what is shown.
- **Manual pre-run step, exact:** before the pass, the owner copies the **main database file
  `data/workspace_app.db`** to a **dated copy, by hand — one copy, explicitly not a backup
  mechanism and not a deletion**. The spec names the path and the exact copy command.
- **Script or UI action:** the spec states which it is. If a script, it names where the script
  lives and states that it is a **tool, not a pytest test** (not collected by `pytest`, not under
  `tests/`), in the spirit of the existing measurement scripts.
