...
- A69. PDF chunks are labelled by PAGE NUMBER; no PDF section title is inferred. The label is a checkable fact about the source, not a claim about its content.
- A70. DOCX chunks keep their real Word-heading section titles, and XLSX chunks keep `sheet_name`; the page-number rule applies to PDFs only.
- A71. The `chunks` table gains a page column via an idempotent migration in the §4.5 style; existing rows have no page until the file is re-ingested.
- A72. The API `sources.page` is populated for PDF sources from the stored page number, while for formats that do not support a page it remains `null` (A59's rule is unchanged for those formats).
- A73. A PDF citation chip displays the stored page number (`Page N`), not an inferred section title.
- A74. Page furniture is removed by repetition only: a line is removed only if it appears on every page of a document of two or more pages, after digit normalisation, and never on a single-page document. Removal never pattern-matches on words.
- A75. Furniture removal never empties a page: if removing furniture would leave a page with no text, that page's text is preserved.
- A76. All five Workspaces are re-ingested in one pass from the original bytes already stored under `data/{workspace_id}/sources/`; nothing under `data/` is deleted or cleared, and the owner re-uploads nothing.
- A77. Every `chunk_id` is regenerated by the pass; a stored citation whose `retrieved_chunk_ids` no longer all resolve still renders without crashing and without silently disappearing or being silently re-pointed, is shown from the stored `display_name`/`section_title` and marked plainly as no longer available because the documents were re-ingested, is not clickable, and falls back to a plain "Source no longer available" label when the stored record cannot identify the source. Any note referring to a chunk by id (including the ids in `memory.md`) is treated as stale.
- A78. The spec names the exact manual pre-run step: copy `data/workspace_app.db` to a dated copy by hand — one copy, not a backup mechanism and not a deletion.
- A79. After re-ingestion, the off-repo positive regression set (referenced by filename only, `GROUNDING_REGRESSION_POSITIVE.md`) still passes at least 14 of its 15 cases when judged on locating phrases rather than chunk ids, and the case whose citation previously pointed at a chunk lacking the answer now resolves to a chunk that contains the cited content.
- A80. The spreadsheet whose rendered row count read 45 against a true 42 reports the true data-row count.
- A81. §22 is ingestion-only: no OCR, no vision-model image handling (F8), no layout-aware parser, no retrieval or prompt change, no chunk-size change, no oversized-chunk splitting, and no new synthetic corpus documents.
- A82. The re-ingestion pass is a script at `scripts/reingest_all.py` — a tool, not a test (not under `tests/`, not collected by `pytest`, asserts nothing) — that the owner runs from his own terminal with the venv active; it prints the document count and the chunk count per Workspace both before and after; it reads the original bytes under `data/{workspace_id}/sources/` and deletes nothing under `data/`.
...
## 22. Ingestion fix bundle

All of this section is `[NEW]` (diagnosed 2026-09-15, written 2026-09-16). **Nothing in this
section is approved to build.** It is written so the project owner can approve or amend it; the
build is a separate, later instruction. It is **ingestion only**: it changes no retrieval
constant, no prompt, and nothing in `retrieval/` or `prompting/`.

### 22.1 Scope, and why all four ship together `[NEW]`

The bundle is four ingestion defects, all diagnosed read-only on 2026-09-15 and recorded in
`memory.md`:

1. xlsx header detection (§22.2)
2. silent content loss at grouping (§22.3)
3. PDF labelling by page number plus page capture (§22.4)
4. page furniture (§22.5)

The four are **independent as CODE** — each can be reasoned about and fixed on its own — but
**inseparable as a RELEASE**. Every one of them requires **re-ingestion** to take effect, and a
**split corpus** — some Workspaces on the new ingestion, some still on the old — contaminates every
later comparison: retrieval, the off-repo positive regression set, and any evaluation would be
measured across two different indexes without saying so. They therefore ship in **one re-ingestion
pass** (§22.6).

### 22.2 Item 1 — xlsx header detection `[NEW]`

**Problem.** `ingestion/parsers.py::parse_xlsx` calls
`pd.read_excel(file_path, sheet_name=None, engine="openpyxl")` with **no `header` argument**, so
pandas' default (`header=0`) applies and **physical row 1 is treated as the column headings**. A
sheet with a title or subtitle block above the real header is read shifted: in one observed case
the rendered count read **45** where the true data-row count was **42**, and the real header became
...
- **OWNER RESERVATION 1 is satisfied explicitly.** Any header-detection rule is a heuristic, and a
  ...
  terms, and (b) what happens when the shape is **ambiguous**.
- **OWNER DIRECTION — ambiguous shape.** Fall back to the current behaviour (**first row as
  header**) **and state the uncertainty in the rendered sheet text**, so that both the reader and
  the model can see the header row was not determined confidently. **Being visibly unsure beats
  being silently wrong.**
- **No identifiable header at all.** The spec defines the behaviour for a sheet in which no row can
  be a header. The sheet is still rendered; it is never dropped and never raises; the rendered text
  says what was assumed.

**Afterwards.** A sheet with a title block above its header renders the **true** number of data
rows, shows a row-number column, and states which header row was used — or states plainly that it
could not be determined confidently.

### 22.3 Item 2 — silent content loss at grouping `[NEW]`

**Problem.** `ingestion/parsers.py:242` — the `if current_texts:` guard inside `flush()` — emits a
section only when body text has accumulated. A **heading immediately followed by another heading**
is therefore **discarded together with its text**. This code lives in
`_group_unstructured_elements`, which the **DOCX path shares**. Measured read-only across 13 corpus
files: **25** drops on the two-column PDF (real content, absent from every chunk of that file),
**13** and **9** on two DOCX files (mostly labels, whose bodies survive under child headings), and
**0** on every xlsx (that path never enters this code).

**Required behaviour — OWNER DIRECTION.**

- **NO TEXT IS EVER DISCARDED AT GROUPING.** This is the invariant of this item; state it as such.
- A heading immediately followed by another heading has its text **carried into the body of the
  next section that is emitted**. It does **not** vanish, and it does **not** create an empty
  section.
- A **trailing heading at end of document** is likewise **preserved**, not dropped.
- The rule applies to **all formats**, because the grouping code is shared.
- **Expected and accepted consequence:** some chunk bodies will gain heading lines as text. This is
  deliberate — **noise is recoverable, loss is not** — and the spec states it so it is not later
  mistaken for a regression.

**Afterwards.** Re-parsing any document yields sections whose combined text contains every
non-empty element; in particular the passages recorded as **dropped in the 2026-09-15 diagnosis
entry in `memory.md`** are present.

### 22.4 Item 3 — PDF labelling by page number, and page capture (MERGED) `[NEW]`

**Problem — two faces of one thing.**

- (a) **PDF section titles are misaligned with their bodies.** The mechanism is recorded on
  2026-09-15 and **corrected for evidence strength the same day**: the element list is **flat**, a
  two-column layout **interleaves** it, and the "most recent heading wins" rule pairs a
  left-column heading with a right-column body. The correct headings **are** found; they are
  attached to the **wrong text**. Tuning `_looks_like_heading` **cannot** fix this — the failure is
  pairing, not detection.
- (b) The `chunks` table stores **nothing page-like**, so the API `sources` field always returns
  `page = null`.

**Required behaviour — OWNER DIRECTION.**

- **STOP inferring section titles for PDFs. Label PDF chunks by PAGE NUMBER.** A page number is a
  **checkable fact**; an inferred section title is a claim that can be wrong, and a wrong citation
  is worse than a plain one because it is **checkable and fails the check**.
- **DOCX keeps its section titles.** Word heading styles are **real structure, not inference**;
  the spec states this distinction explicitly.
- **XLSX keeps `sheet_name`** as today.
- **Capture the page number during parsing and store it on the chunk.** The schema change to the
  `chunks` table and the migration are specified in the style of §4.5 (idempotent; safe on a fresh
  DB and safe to run repeatedly). **Existing rows have no page until re-ingested.**
- **Citation chip for a PDF chunk** displays the stored page label (e.g. `Page N`), not an inferred
  section. The spec states the exact label form.
- **API `sources.page`:** for PDF sources it now returns the stored page number; for formats that
  do not support a page it **remains `null`**. **A59 is NOT edited** — the new criteria state the
  PDF change and leave A59's rule intact for the other formats.
- **A layout-aware PDF parse is REJECTED** — heavy native Windows dependencies, the same ground OCR
  was rejected on. **Record that this rejection is specific to a layout-aware PARSER and does NOT
  revive the old OCR reasoning against the vision-model route (F8)**, which remains **deferred by
  direction**.
- **A29 dependency.** A29 is recorded as **partially discharged** and now has **evidence against it
  for PDFs**. **A29 is NOT edited here.** The spec states the dependency — PDF section-title
  quality is superseded for PDFs by page labelling — and **leaves the decision to the owner**.

**Afterwards.** Every PDF chunk carries the page number it came from; its citation shows that page;
the API returns that page; DOCX and XLSX are unchanged.

### 22.5 Item 4 — page furniture `[NEW]`

**Problem — two faces of one thing:** footer text **bleeding into chunk bodies**, and running
headers **passing the heading test** and becoming section titles.

**Required behaviour — OWNER DIRECTION on the rule**, because this is the third heuristic in this
bundle and the previous two have bitten:

- A line is removed as furniture **ONLY IF it appears on EVERY page** of a document of **two or
  more pages**, **after normalising digits** so that lines differing only by a page number count as
  the same line.
- **Repetition across pages is the evidence.** Do **not** pattern-match on words such as "Page",
  "Internal use", or "Confidential" or similar — that is guessing.
- **Never apply furniture removal to a single-page document.**
- This item **DEPENDS on item 3**: without page numbers there is no way to know what "every page"
  means. The dependency is stated, and the two travel together.
- **If removal would empty a page entirely**, the furniture is **RETAINED for that page** — one
  rule, no alternatives — so that "no text discarded" (§22.3) is not violated; the page's text is
  not discarded.

**Afterwards.** Repeated page furniture no longer appears in chunk bodies or as section titles; a
single-page document is untouched; lines differing only by digits are treated as the same line.

### 22.6 The re-ingestion pass `[NEW]`

- **All five Workspaces are re-ingested in one pass:** `aml-workspace`,
  `62ee0cd0d2324cd5ab5e20b70a8b27bc`, `aa6dc06b19dd47e1a24d4855b951d24f`,
  `524bf0ec5c5f4290a80438fd801455ce`, `d93d5cd12269400e9298c535062afc65`.
- Re-ingestion reads the **original source bytes already stored under
  `data/{workspace_id}/sources/`**. The owner **does not re-upload anything**, and **nothing under
  `data/` is deleted or cleared at any point** (AGENTS.md rule 7).
- **Every `chunk_id` is regenerated.** Any note that refers to a chunk by id — **including the ids
  recorded in `memory.md`** — becomes stale.
- **What a stored citation actually contains** (inspected read-only 2026-09-16). Each assistant
  message stores `cited_sources`: a JSON list of `{source_id, display_name, section_title}`
  (`section_title` may be `null`); and, separately, `retrieved_chunk_ids`: a JSON list of chunk-id
  strings. The retrieved ids are **index-aligned** with the citation list, are **not embedded per
  citation**, and are **not rendered today**. The chip rendering (`ui/cards.py::chips_html`) uses
  `display_name` + `section_title` only. The stored record therefore carries **no page and no
  per-citation chunk id**.
- **Stale-citation behaviour — OWNER DIRECTION.** When rendering a stored assistant message whose
  `retrieved_chunk_ids` no longer all resolve in the current `chunks` table, the UI must: **still
  render** the message's sources (**must not crash**, **must not silently disappear**); render each
  from the identifying information **actually stored** (`display_name`, and `section_title` if
  present); mark the set plainly as **no longer available because the documents were re-ingested**;
  and **not** make them clickable. It must **not** silently re-point a stale id at a different
  chunk.
- **Minimum visible fallback.** If the stored record cannot identify a source (`display_name`
  missing or empty, and no `section_title`), the chip falls back to a plain **"Source no longer
  available"** label. **The spec does not invent a field for this.** Because the stored record
  carries no page, a **PDF citation stored before this bundle cannot show a page** after
  re-ingestion.
- **Manual pre-run step, exact.** Before the pass, from the repository root, the owner runs — with
  the date of the pass substituted for `YYYY-MM-DD`:

  ```
  Copy-Item data\workspace_app.db "data\workspace_app_YYYY-MM-DD.pre-reingest.db"
  ```

  **One copy, made by hand; explicitly not a backup mechanism and not a deletion of the original.**
- **Script, not a UI action — OWNER DIRECTION.**
  - The pass is a **script**, at **`scripts/reingest_all.py`**, in the spirit of the existing
    measurement scripts (`scripts/measure_open16.py`, `scripts/run_question_set.py`).
  - It is a **TOOL, NOT A TEST**: not under `tests/`, not collected by `pytest`, and it **asserts
    nothing**.
  - The owner runs it from his own terminal with the **venv active**.
  - It prints, **per Workspace, the document count and the chunk count BEFORE and AFTER**, so the
    owner can see at a glance that nothing vanished.
  - It reads the original bytes under `data/{workspace_id}/sources/` and **deletes nothing under
    `data/`**.
  - It is **not written now** — §22 specifies it; the build is a separate instruction.

### 22.7 How we know it worked `[NEW]`

- **Before/after — the off-repo positive regression set is the acceptance check.** Referenced by
  **filename only** (`GROUNDING_REGRESSION_POSITIVE.md`); it is off-repo and must **not** be read
  into or committed to this repository. Its current state is **15 questions, 14 passing**. **After
  re-ingestion, at least 14 of 15 must still pass, judged on locating phrases rather than chunk
  ids**, since the ids change.
- **Corrected citation.** One known case currently passes with a citation pointing at a chunk that
  does **not** contain the answer. After the pass, that citation must resolve to a chunk that
  **does** contain the cited content.
- **Content recovery.** The passages recorded as **dropped in the 2026-09-15 diagnosis entry in
  `memory.md`** must, after re-ingestion, each appear in **at least one chunk of the file they came
  from**. The requirement is stated here; the **concrete list of passages stays off-repo**.
- **Row counts.** The spreadsheet whose rendered row count read **45** against a true **42** must
  report the **true** data-row count.
- **The owner runs these checks by hand.** They are not automated, and must not be automated by
  reading the off-repo set into the repository.

### 22.8 Explicitly out of scope for this section `[NEW]`

At minimum: no OCR; no vision-model image handling (F8 stays deferred by direction); no
layout-aware PDF parser; no retrieval or prompt changes; no chunk-size changes; no splitting of
oversized chunks; no new synthetic corpus documents. The **methodology finding** — that the
synthetic corpus cannot exhibit this class of defect — is **recorded and deliberately NOT acted on
here**.
